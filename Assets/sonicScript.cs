...
public class sonicScript : MonoBehaviour
{
    ...
    private static List<sonicScript> sonics = new List<sonicScript>();
    ...
    {
        if (displayedImage.GetComponent<Renderer>().enabled)
        {
            displayedImage.GetComponent<Renderer>().enabled = false;
            Reset();
            Begin();
        }
        var s = Bomb.QueryWidgets(KMBombInfo.QUERYKEY_GET_SERIAL_NUMBER, null);
        var modules = Bomb.GetModuleNames().FindAll(x => x == "Sonic the Hedgehog").Count;
        serial = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, string>>(s[0])["serial"];
        sonics.Add(this);
        var j = 0;
        var c = System.Linq.Enumerable.Count(sonics);
        yield return new WaitUntil(() => {
            j++;
            return System.Linq.Enumerable.Count(System.Linq.Enumerable.Where(sonics, x => x.serial == serial)) == modules || (j > 30 && System.Linq.Enumerable.Count(sonics).Equals(c));
            });
        var list = System.Linq.Enumerable.ToList(System.Linq.Enumerable.Where(sonics, x => x.serial == serial));
        var index = list.IndexOf(this);
        onButton.OnInteract();
        yield return new WaitForSeconds(2);
        if (index != 0) yield return new WaitUntil(() => list[index - 1].pass1);
        boots.OnInteract();
        yield return null;
        invincible.OnInteract();
        yield return null;
        life.OnInteract();
        yield return null;
        rings.OnInteract();
        pass1 = true;
        yield return new WaitUntil(() => list.TrueForAll(x => x.pass1));
        yield return new WaitForSeconds(4);
        startButton.OnInteract();
        yield return new WaitForSeconds(5);
        var picNum = new[] { pic2, pic3 };
        var rendNum = new[] { pass1rend, pass2rend };
        for (int i = 0; i < 2; i++)
        {
            displayedImage.GetComponent<Renderer>().material.mainTexture = picNum[i];
            Audio.PlaySoundAtTransform("ringCollect", transform);
            rendNum[i].GetComponent<Renderer>().enabled = true;
            stage++;
            yield return new WaitForSeconds(0.3f);
        }
        pass2 = true;
        yield return new WaitUntil(() => list.TrueForAll(x => x.pass2));
        displayedImage.GetComponent<Renderer>().material.mainTexture = pic4;
        Audio.PlaySoundAtTransform("gameOver", transform);
        pass3rend.GetComponent<Renderer>().enabled = true;
        pass3 = true;
        if (System.Linq.Enumerable.Count(System.Linq.Enumerable.Where(sonics, x => !x.pass3)) == 1) sonics.Clear();
        Debug.LogFormat("[Sonic the Hedgehog #{0}] The module has been disarmed using an unsupported gameshark code.", moduleId);
